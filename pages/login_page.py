...
class LoginPage(BasePage):
    ...
    def get_user_form(self):
        return self.driver.find_element(*self.locators.USER_NAME_LOCATOR)

    def fill_in_user(self, user):
        self.get_user_form().send_keys(user)

    def get_pwd_form(self):
        return self.driver.find_element(*self.locators.PWD_LOCATOR)

    def fill_in_pwd(self, password):
        self.get_pwd_form().send_keys(password)

    def click_login_button(self):
        self.driver.find_element(*self.locators.LOGIN_BUTTON_LOCATOR).click()
    ...
    def find_error_frame(self):
        return self.driver.find_element(*self.locators.ERROR_CONTAINER_LOCATOR)

    def get_error_message(self):
        return self.find_error_frame().get_attribute('innerText')

    def click_error_cross_button(self):
        self.driver.find_element(*self.locators.ERROR_CROSS_BUTTON).click()
    ...
    def wait_for_delay_user_to_log_on(self):
        return WebDriverWait(self.driver, 1).until_not(EC.visibility_of_element_located(self.locators.LOGIN_BUTTON_LOCATOR))
